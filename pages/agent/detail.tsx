...
const AgentDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
	...
	useEffect(() => {
		if (commentInquiry.search.commentRefId) {
			getCommentsRefetch({ varibale: { input: commentInquiry } }).then();
		}
	}, [commentInquiry]);
	...
	const createCommentHandler = async () => {
		try {
			...
			if (user._id === agentId) throw new Error('You cannot write a review for yourself');
	...
	const likePropertyHandler = async (user: any, id: string) => {
		try {
			...
			sweetTopSmallSuccessAlert('succes', 800);
		} catch (err: any) {
